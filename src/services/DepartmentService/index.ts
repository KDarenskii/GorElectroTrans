...
export default class DepartmentService {
    ...
    static post = ({ depId, department }: PostParams, config?: AxiosRequestConfig): Promise<AxiosResponse<any>> => {
        return api.post<any>(`/dep_${depId}/subdep/data`, department, config);
    };
    static put = ({ depId, department }: PutParams, config?: AxiosRequestConfig): Promise<AxiosResponse<void>> => {
        return api.put<void>(`/dep_${depId}/subdep/${department.id}`, department, config);
    };
    static delete = (
        { depId, departmentId }: DeleteParams,
        config?: AxiosRequestConfig
    ): Promise<AxiosResponse<void>> => {
        return api.delete(`/dep_${depId}/subdep/${departmentId}`, config);
    };
}
